(function () {
  ...
  function toggleAutoPlay() {
    ...
    function playDonation(offsetT) {
      ...
      step(() => openDonationPopup(), offsetT + 1800);
      step(() => stopAutoPlay(), offsetT + 2200);
    }
    ...
    if (popupCopyUpiBtn) {
      popupCopyUpiBtn.addEventListener("click", () => {
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText("Q178007075@ybl").catch(() => {});
        }
        showToast("✅ Copied UPI ID: Q178007075@ybl");
      });
    }
  ...
  Promise.all([preload()]).then(() => {
    ...
    if (params.has("autoplay") || params.get("mode") === "ar") {
      window.setTimeout(() => {
    ...
    window.addEventListener("message", (e) => {
      ...
      if (act === "startAutoplay" && !isAutoPlaying) {
        toggleAutoPlay();
      } else if ((act === "stopAutoplay" || act === "pauseAutoplay") && isAutoPlaying) {
        stopAutoPlay();
      } else if (act === "toggleAutoplay") {
        toggleAutoPlay();
      } else if (act === "playMusic" || act === "startMusic") {
        TempleAudio.startMusic();
      } else if (act === "pauseMusic" || act === "stopMusic") {
        TempleAudio.pauseMusic();
      } else if (act === "toggleMusic") {
        TempleAudio.toggleMusic();
      }
    });
  });
})();
